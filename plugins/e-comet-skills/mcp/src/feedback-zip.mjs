...
const writeLocalHeader = ({ nameBytes, bytes, crc }) => {
...
const writeCentralHeader = ({ nameBytes, bytes, crc, localOffset }) => {
...
export const createFeedbackZip = ({ reportBytes, metadataBytes, transcriptBytes } = {}) => {
    ...
    const preparedEntries = entries.map((entry) => ({ ...entry, nameBytes: Buffer.from(entry.name, 'utf8') }));
    const localBytes = preparedEntries.reduce((total, entry) => total + 30 + entry.nameBytes.length + entry.bytes.length, 0);
    ...
    const localParts = [];
    const centralParts = [];
    for (const { bytes, nameBytes } of preparedEntries) {
        const crc = crc32(bytes);
        const localHeader = writeLocalHeader({ nameBytes, bytes, crc });
        localParts.push(localHeader, nameBytes, bytes);
        centralParts.push(writeCentralHeader({ nameBytes, bytes, crc, localOffset }), nameBytes);
