    ...
    try {
        ...
        for (let attempt = 0; attempt < CACHE_REPLACE_RETRY_LIMIT; attempt += 1) {
            try {
                ...
                if (!TRANSIENT_REPLACE_ERRORS.has(error?.code) || attempt === CACHE_REPLACE_RETRY_LIMIT - 1) throw error;
...
export const releaseOwnedLock = async ({ lockPath, ownerPath, operations = {} }) => {
...
const acquireLock = async (lockPath, nowMs) => {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
            ...
            try {
                ...
                return () => releaseOwnedLock({ lockPath, ownerPath });
            } catch (error) {
...
const resolveLatestVersion = async ({ dataRoot, fetchImpl, nowMs }) => {
    ...
    const release = await acquireLock(lockPath, Date.now());
    if (release === null) {
        if (cached?.latestVersion) return { fatal: false, deferred: false, cached };
        const deadline = Date.now() + GLOBAL_LOCK_WAIT_MS;
        while (Date.now() < deadline) {
...
export const checkUpdateForSession = async ({ pluginRoot, dataRoot, event, fetchImpl, nowMs }) => {
    try {
        ...
        const release = await acquireLock(join(sessionsRoot, `${key}.lock`), Date.now());
