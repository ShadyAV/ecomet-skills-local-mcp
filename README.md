...
> [!IMPORTANT]
> Тестовая сборка плагина: локальный MCP с актуальными исправлениями и продовый удалённый MCP `https://mcp.e-comet.io/mcp`.
> Удалённые инструменты работают с вашим настоящим аккаунтом e-Comet; это не stage-окружение. Отдельно подключать stage MCP не нужно.
...
Рекомендуем регулярно обновлять установку для добавления нового функционала и улучшения стабильности текущего. При использовании функций полной установки агент может показать короткую заметку об обновлении после основного результата. Во время отправки баг-репорта проверка откладывается до следующей обычной операции. Список последних изменений доступен в [CHANGELOG](CHANGELOG.md).
