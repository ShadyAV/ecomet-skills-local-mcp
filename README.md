...
> [!IMPORTANT]
> Этот тестовый marketplace устанавливает только локальный MCP. Для тестирования подключите stage MCP отдельно.
> В конфигурации stage MCP для Codex укажите `omit_tools_from = ["code_mode", "deferred"]`, чтобы e-Comet вызывался напрямую и trusted lifecycle выполнялся полностью.
...
### Сообщить о проблеме e-Comet

Если инструмент e-Comet завершился ошибкой, вернул неверные данные или не помог выполнить задачу, попросите агента сообщить об этом в e-Comet — например: «Сообщи о проблеме e-Comet».

Агент подготовит отчёт с описанием ситуации и технической информацией о работе инструментов e-Comet. Перед отправкой он отдельно спросит, можно ли приложить точную историю текущей беседы: она поможет разобраться, но может содержать чувствительные данные. Отчёт можно отправить без истории беседы или отменить отправку.
