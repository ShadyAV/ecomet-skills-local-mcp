...
Агент подготовит отчёт только по уже имеющимся в беседе сведениям и наблюдаемым результатам e-Comet. Если сведения известны, в нём будут затронуты инструмент или операция, фактический и ожидаемый результаты, контекст воспроизведения и предпринятые попытки восстановления; безопасные код и сообщение ошибки сохраняются без изменений. Агент не выдумывает причину, опускает неизвестное и не задаёт дополнительные вопросы только ради заполнения отчёта.

Если вы ещё не выбрали вариант отправки, агент один раз пояснит, что отчёт содержит фактическое описание проблемы, локальную диагностику и базовую информацию об окружении. Затем предложит ровно три варианта: отправить без истории текущей сессии, отправить с полной историей текущей сессии или не отправлять. Полная история содержит больше, чем видимый чат, и может включать системный и служебный контекст, вызовы инструментов и их результаты, учётные данные, персональные или коммерческие данные, исходный код, пути к файлам, несвязанное содержимое и другой контекст.

Однозначный выбор в исходном запросе уже считается согласием, поэтому агент сразу начинает подготовку без повторного пояснения. После однозначного выбора в следующем сообщении подготовка также начинается сразу. При неясном ответе агент задаёт один короткий уточняющий вопрос, не повторяя всё пояснение; при отказе инструменты отправки не вызываются.
