...
Перед отправкой агент предложит ровно три варианта: отправить без приложения; отправить с точной текущей расшифровкой — предоставленным хостом необработанным техническим транскриптом текущего вложения; или отменить отправку. Такой транскрипт не ограничен видимыми сообщениями чата и может содержать сообщения, системные и developer-инструкции, вызовы инструментов и их результаты, учётные данные, персональные или коммерческие данные, исходный код, пути к файлам, несвязанное содержимое и другой контекст хоста. Явный выбор «с транскриптом» или «без транскрипта» в текущем сообщении уже считается согласием; при неясном ответе агент не отправляет отчёт.
